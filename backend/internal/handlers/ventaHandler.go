...
func CreateVentaHandler(w http.ResponseWriter, r *http.Request) {
	...
	var id int
	// Invocación del 5to Stored Procedure con manejo de transacción en el DBMS
	err := gormDB.Raw("SELECT sp_registrar_venta(?, ?, ?, ?)",
		req.IDCliente, req.IDEmpleado, req.FechaVenta, req.Precio).Scan(&id).Error
