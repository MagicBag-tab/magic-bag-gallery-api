...
import { getArtistas, getArtista } from '../../api/api';
...
export default function Artists() {
  ...
  const handleSelect = async (a) => {
    ...
    try {
      const detail = await getArtista(a.id_artista);
      setSelected(detail);
    } catch {
      setSelected(a);
    } finally {
    ...
    <div className={styles.page}>
      ...
      {loading ? <Loader fullPage /> : (
        <div className={styles.grid}>
          {artistas.map((a, i) => (
            <article key={a.id_artista} className={styles.card} style={{ animationDelay: `${i * 0.06}s` }} onClick={() => handleSelect(a)}>
              <div className={styles.avatar}><span>{initials(a.nombre_completo)}</span></div>
        ...
        <Modal title="Artista" onClose={() => setSelected(null)}>
          {loadingDetail ? <Loader /> : (
            <div className={styles.detail}>
              <div className={styles.detailAvatar}><span>{initials(selected.nombre_completo)}</span></div>
              ...
              {selected.colecciones?.length > 0 && (
                <div className={styles.detailSection}>
                  ...
                  <ul className={styles.list}>{selected.colecciones.map(c => <li key={c}>{c}</li>)}</ul>
                </div>
              )}
            </div>
          )}
